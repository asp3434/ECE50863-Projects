...
current_bitrate: int
startup_flag: bool
buffer_hysteresis: int

def customABR(message: ClientMessage):
    # variables from Robust MPC
    global window
    global num_chunks
    ...
    global startup_flag
    global buffer_hysteresis
    
    ##### starting phase #1 #####
    # initialize variables
    if len(time_array) <= 1:
        num_chunks = len(message.upcoming_quality_bitrates) + 1
        target_buffer = round(0.05 * num_chunks)
        buffer_hysteresis = round(target_buffer *0.1)
        current_bitrate = 0
        startup_flag = True
    ...
    # default to build buffer queue for 2 chunks using minimum bitrate quality
    if len(time_array) <= 3:
        update_bitrate_array(message, 0)
        return 0
    
    ##### starting phase #2 ##### 
    # maximize bitrate and hit the target buffer as fast as possible
    elif (buffer_size_array[-1] < target_buffer) and startup_flag: 
        proposed_bitrate = len(message.quality_bitrates)-1
        
        # only increase the quality if it grows the buffer size
        check = buffer_shrink_check(message, proposed_bitrate)
        while check:
            proposed_bitrate = max(0, proposed_bitrate-1)
            if proposed_bitrate == 0:
                break
    ...
    # buffer size is MORE than the target buffer by more than the set hysteresis value
    elif buffer_size_array[-1] > target_buffer + buffer_hysteresis:
        temp_quality = len(message.quality_bitrates)-1
        update_bitrate_array(message, temp_quality)
        return temp_quality
    
    # buffer size is LESS than the target buffer by more than the set hysteresis value
    elif buffer_size_array[-1] < target_buffer - buffer_hysteresis:
        proposed_bitrate = len(message.quality_bitrates)-2
    ...
    # buffer size is at the target buffer size (given some hysteresis), use RobustMPC      
    else:
        n = 0
        qoe = 0.00001
...
def buffer_shrink_check(message: ClientMessage, proposed_bitrate):
    download_time = message.quality_bitrates[proposed_bitrate]/predicted_throughput_array[-1]
    if download_time > message.buffer_seconds_per_chunk:
        return 1
    else:
        return 0
...
#     plt.savefig("customABR_buffer_size.png")
#     plt.close()
...
#     plt.savefig("customABR_bitrate_quality.png")
#     plt.close()
...
#     plt.savefig("customABR_throughput.png")
#     plt.close()
...
def student_entrypoint(client_message: ClientMessage):
    ...
    quality = customABR(client_message)
